...
/*The goal here is to make bad pathing for the API to catch if Respector
    Will still handle proper routes as necessary*/
@Provider
@Path("/badtest")
@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
public class badTestRestAPI {
    private static final Logger log = LogManager.getLogger(badTestRestAPI.class);
    ...
    @GET
    @Path("findgradehigher/{gradeNumber}")
    ...
    @GET
    @Path("findbytestname/{gradeNumber}")
    public Object getGradeLower(@PathParam("gradeNumber")String gradeNum){
        log.info("Get Test Name"+ gradeNum);
        if (gradeNum.isEmpty()){
            return getResponse(Status.BAD_REQUEST);
        }
        List<TestClass> Tests = TestService.getInstance().findTestName(gradeNum);
        if(Tests.isEmpty()){
            return getResponse(Status.NOT_FOUND);
        }
        return Tests;
